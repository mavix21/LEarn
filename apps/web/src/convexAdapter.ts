...
export const ConvexAdapter: Adapter = {
  ...
  async createUser({ id: _, ...user }: User & { address: string }) {
  ...
  async deleteUser(id: Id<"users">) {
    maybeUserFromDB(await callMutation(api.authAdapter.deleteUser, { id }));
  },
  ...
  async getSessionAndUser(sessionToken: string) {
    ...
    return {
      user: userFromDB(result.user) as AdapterUser,
      session: sessionFromDB(result.session) as AdapterSession,
    };
  },
  async getUser(id: string) {
    const user = await callQuery(api.authAdapter.getUser, {
      id: id as Id<"users">,
    });
    if (!user?.email) return null;
    return userFromDB(user) as AdapterUser;
  },
  async getUserByAccount({ provider, providerAccountId }) {
    const user = await callQuery(api.authAdapter.getUserByAccount, {
      provider,
      providerAccountId,
    });
    if (!user?.email) return null;
